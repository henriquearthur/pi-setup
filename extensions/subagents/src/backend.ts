/**
 * The backend interface: a `SubagentBackend` produces `SubagentSession`s of
 * one normalized shape. The only wired backend is pi: in-process
 * `createAgentSession()` via the pi SDK.
 */
...
export interface SubagentBackend {
  ...
  /** Probe availability (SDK importable, credentials). */
