...
export const SUBAGENT_SPAWN_TOOL_DESCRIPTION =
  "Spawn a background subagent: a fully autonomous, headless agent running as an in-process pi session with its own context window and this environment's normal tools and config. Fire-and-forget: this returns immediately with an id. The subagent's final output is queued back to you as a message when it settles, or collect it explicitly with subagent_wait. Children cannot orchestrate more agents/workflows or ask the user, and cannot see this conversation, so the prompt must be self-contained. Only use trusted working directories. Max 4 subagents can be running at once.";
...
export const SUBAGENT_SPAWN_PARAMETER_DESCRIPTIONS = {
  ...
  model:
    'Model hint: "provider/model-id" or a bare model id. Omit to inherit the current model.',
  reasoningEffort:
    "Reasoning effort, mapped to pi thinking levels. Omit to inherit the current level.",
};
...
export function buildSubagentSpawnResult(options: {
  ...
  return (
    `Spawned subagent ${options.id} "${options.title}" (pi: ${options.modelLabel}, ${options.cwd}).\n` +
